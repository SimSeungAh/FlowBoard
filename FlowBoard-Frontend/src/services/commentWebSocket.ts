...
export type CommentConnectionState = "connecting" | "connected" | "disconnected";
...
interface ConnectCommentWebSocketOptions {
  ...
  onEvent: (event: CommentWebSocketEvent) => void;

  onConnectionStateChange?: (state: CommentConnectionState) => void;
...
export const connectCommentWebSocket = ({
  ...
  const subscriptionId = `comments-${boardId}`;

  const changeConnectionState = (state: CommentConnectionState) => {
  ...
  const subscribe = () => {
    ...
    socket.send(
      createStompFrame("SUBSCRIBE", {
        ...
        destination: `/topic/boards/${boardId}/comments`,
  ...
  const handleMessageFrame = (frame: ParsedStompFrame) => {
    try {
      const event = JSON.parse(frame.body) as CommentWebSocketEvent;
      ...
      reportError(new Error("댓글 실시간 데이터를 해석하지 못했습니다."));
    }
  };
  ...
  const handleStompFrame = (rawFrame: string) => {
    ...
    switch (frame.command) {
      ...
      case "ERROR":
        ...
        reportError(new Error(frame.body || "댓글 WebSocket 서버 오류가 발생했습니다."));
  ...
  const connect = () => {
    ...
    socket.onerror = () => {
      reportError(new Error("댓글 WebSocket 연결 중 오류가 발생했습니다."));
    };
